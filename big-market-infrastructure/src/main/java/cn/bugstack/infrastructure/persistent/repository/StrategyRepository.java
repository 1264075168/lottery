...
public class StrategyRepository implements IStrategyRepository {
    ...
    @Override
    public int getRateRange(String key) { //先判断一下是否已经装配了
...
//===================前置规则查询======================
    ...
    public RuleTreeVO queryRuleTreeVOByTreeId(String treeId) {
        ...
        for (RuleTreeNodeLine ruleTreeNodeLine : ruleTreeNodeLines) {
            ...
            // 存放起始节点(ruleNodeFrom)和对应的线
            List<RuleTreeNodeLineVO> ruleTreeNodeLineVOList = ruleTreeNodeLineMap.computeIfAbsent(ruleTreeNodeLine.getRuleNodeFrom(), k -> new ArrayList<>());
            ruleTreeNodeLineVOList.add(ruleTreeNodeLineVO);
        }
        // 2. tree node 转换为Map结构 //把对应的RuleKey作为键，节点作为值
    ...
    public void cacheStrategyAwardCount(String cacheKey, Integer awardCount) {
        if (redisService.isExists(cacheKey)) return;//避免重复装配
    ...
    @Override //创建一个队列信息，然后把他交到一个延迟队列
    public void awardStockConsumeSendQueue(StrategyAwardStockKeyVO strategyAwardStockKeyVO) {
        String cacheKey = Constants.RedisKey.STRATEGY_AWARD_COUNT_QUERY_KEY;//定义一个队列key
